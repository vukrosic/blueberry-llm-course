...
def main():
    ...
    # Sweep configurations: Experts
    configs_to_test = [
        {"name": "baseline_8e_2k", "num_experts": 8,  "expert_top_k": 2},
        {"name": "dense_4e_2k",    "num_experts": 4,  "expert_top_k": 2},
        {"name": "sparse_16e_2k",  "num_experts": 16, "expert_top_k": 2},
        {"name": "top1_8e_1k",     "num_experts": 8,  "expert_top_k": 1},
    ]
    ...
    print("-" * 80)
    for res in results:
        cfg_str = f"E={res['config']['num_experts']}, K={res['config']['expert_top_k']}"
        print(f"{res['name']:<20} | {res['val_loss']:<10.4f} | {res['val_ppl']:<10.2f} | {cfg_str}")
    
    # Save results to file
    import json
    with open("experiments/expert_sweep_results.json", "w") as f:
        json.dump(results, f, indent=2)
    print(f"\nResults saved to experiments/expert_sweep_results.json")
